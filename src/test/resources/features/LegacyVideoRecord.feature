...
@Regression
  ...
  |ashokmarch12@yopmail.com|Password@123|
    ...
    When the user is on the Legacy Messages page
...
@Regression @RecordAudio
 Scenario: Legacy Audio Record Workflow
 Given User is already logged in to application
  |username            |password    |
  |ashokmarch12@yopmail.com|Password@123|
  ...
  When the user is on the Legacy Messages page
    ...
    And the user clicks on Create your first message button
    And the user click on the record One Here link
    And the user clicks on Record Audio message button
    And the user clicks on Record now button
    And the user clicks on start recording button
